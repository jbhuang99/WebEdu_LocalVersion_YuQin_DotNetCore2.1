...
description: '使用 C# 脚本对字符串进行高级处理（如反转、格式转换）。当用户要求处理字符串、反转文本或进行特定字符操作时使用'
...
2. **执行脚本**：
   ...
   - **执行命令**：运行位于 `scripts/ProcessString.cs` 的脚本。
   - **命令示例**：
     ```bash
     dotnet run --file scripts/ProcessString.cs "用户提供的原始字符串"
     ```
3. **处理输出**：
   - 读取脚本的标准输出（stdout）。
...
## 注意事项
- 确保在调用命令时，当前工作目录位于技能根目录下，以保证相对路径 `scripts/ProcessString.cs` 能够正确识别。
