...
（A）	.razor.cs文件的AOT编译的SPA单页应用，可以作为功能强大的主导框架（一次性耗时下载到客户端但可PWA）。.html/.svg/.x3d/.css/.js文件的JIT编译的MPA多页应用，可以作为功能小巧的辅助填充（分批性省时下载到客户端无需PWA。此时，期待平台能够内置提供如下功能：.cs文件的JIT编译的C#代码→（JIT转换成为）→.js文件的JIT编译的JS代码，这样，此类视图V，可以完全JIT编译的C#编程，从而无需依赖JIT编译的JS编程，请参见https://github.com/SharpKit/SharpKit）。
（B）	便于统一C#编程处理.docx/.xslx/.pptx等等各种日常文件。
（C）	不便当前的搜索引擎收录（但是可以他方引流解决）。
（D）	功能兼容超越下述的“视图V之二”、“视图V之三”。
（E）	“视图V”C#编程思维、“控制C/模型M”C#编程思维，即，“MVC”统一C#编程思维。
