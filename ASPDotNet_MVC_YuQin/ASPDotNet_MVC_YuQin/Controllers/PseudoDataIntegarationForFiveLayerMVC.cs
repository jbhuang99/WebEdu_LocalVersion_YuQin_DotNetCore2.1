...
{
    public class PseudoDataIntegarationForFiveLayerMVCController : Controller
    {
        public PseudoDataIntegarationForFiveLayerMVCController(
        ...
        {
            // sFileName准备用作文件读取；
            return Content("<HTML><HEAD><TITLE>课文</TITLE><BODY><div style='width:100%;height:100%;text-align:center;vertical-align:middle;'><span>(底层第1层MVC) 实践(数据读写封装)【默认标量的媒体字面顺序匹配，理解向量/矩阵/张量的媒体Token语义距离】：数据集成的案例（面向数据读写封装的数据库等等众多源头的原始数据→基于SS的SSIS等等服务→面向数据只读统计的数据仓库数据）（当前还需在VS/SSMS/SS中预实现后调用）</span></div></BODY ></HTM>", "text/html", System.Text.Encoding.UTF8);//不知为什么，必须""嵌套''，否则提示“字符文本中字符太多”错误
        }
