...
{
    public class PseudoDataDescriptionAnalysisForFiveLayerMVCController : Controller
    {
        public PseudoDataDescriptionAnalysisForFiveLayerMVCController(
        ...
        {
            // sFileName准备用作文件读取；
            return Content("<HTML><HEAD><TITLE>课文</TITLE><BODY><div style='width:100%;height:100%;text-align:center;vertical-align:middle;'><span>(中层第2层MVC) 技术(信息提取运用)【默认标量的媒体字面顺序匹配，理解向量/矩阵/张量的媒体Token语义距离】：数据描述统计的案例（当前还需在VS/SSMS/SS中预实现后调用）</span></div></BODY ></HTM>", "text/html", System.Text.Encoding.UTF8);//不知为什么，必须""嵌套''，否则提示“字符文本中字符太多”错误
        }
