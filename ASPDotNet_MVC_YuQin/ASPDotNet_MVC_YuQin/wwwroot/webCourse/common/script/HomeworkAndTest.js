  ...
  var oTable = document.getElementsByTagName("table").item(0);var cTr = Array.from(oTable.rows); // 只包含主表的直接<tr>。解决上述包含嵌套表格的行。
   ...
   var iProblemNum = iTrLenth / 9;
    ...
    for (i = 0; i < iTrLenth; i++) {
        //for (j = 0; j < 6; j=i*j) {
        if ((i + 9) % 9 === 0) { cTr[i].getElementsByTagName("td").item(0).innerHTML = "<span style=\"display:inline-block; padding-top: 12px;padding-bottom: 12px;color:red\">题目" + ((i + 9) / 9) + "：<input type=\"button\" style=\"white-space: normal; text-align: justify\" value=\"单击标题栏的“内容切换”可以切换课文的显示\" title='【“目录导航的课文”↔“面向一对一的学生课外自主学习”↔“面向一对多的教师课堂答疑教学”（“目录导航的课文”建议主选“作业测验”采集数据、“面向一对一的学生课外自主学习”建议主选“作业测验”实施学习、“面向一对多的教师课堂答疑教学”建议主选“作业测验”的“答案解释”链接实施教学的字符文档/图像PPT/视频/2D/3D的视媒/听媒/触媒/嗅媒/味媒）】' /><input type=\"button\" style=\"white-space: normal; text-align: justify\" value=\"单击切换本题显示\" title='以便聚焦显示的题目，以便排除其他题目干扰！' onclick=\"fnToggleItemDisplay("+i+")\" /><input type=\"button\" style=\"white-space: normal; text-align: justify\" value=\"单击切换本题在线编辑\" title='当前只是临时应急排版错误，但是无法保存编辑结果，必须重新上传当前条目的作业测验的.doc/.docx文档（作业测验的模板建议单击“标题框架”的“关于条目资源模板”下载）。后续将开发保存编辑结果的功能！' onclick=\"fnToggleContentEditable("+i+")\" /><input type=\"button\" style=\"white-space: normal; text-align: justify\" value=\"单击登录服务端的作业测验的“四层平台”的思维语言生成的“五层MVC”的统计分析（正在迁移开发之中...）\" onclick=\"fnHomeworkAndTestFiveLayerMVC()\" /></span>" + cTr[i].getElementsByTagName("td").item(0).innerHTML; }
        ...
        if ((i + 9) % 9 === 5) {cTr[i].getElementsByTagName("td").item(0).innerHTML = "<span><span style=\"color:red\"><input type=\"button\" style=\"white-space: normal; text-align: justify\" title='（本机端作业测验不支持成绩的“四层平台”思维语言生成的“五层MVC”的统计分析，服务端作业测验支持成绩的“四层平台”思维语言生成的“五层MVC”的统计分析！）（本机端作业测验←可以登录服务端同步→服务端作业测验！）' onclick=\"fnAnswerAndToggoleAnswerDisplay();\" value=\"本题答案 【注：（本机端作业测验不支持成绩的“四层平台”思维语言生成的“五层MVC”的统计分析，服务端作业测验支持成绩的“四层平台”思维语言生成的“五层MVC”的统计分析！）（本机端作业测验←可以登录服务端同步→服务端作业测验！）】\" /></span>" + "<span style=\"display:none\" id=\"idAnswer" + sTimeStamp + Math.floor(((i + 9) / 9))  +"\""+">" + cTr[i].getElementsByTagName("td").item(0).innerHTML + "</span></span>";}
       if ((i + 9) % 9 === 6) { cTr[i].getElementsByTagName("td").item(0).innerHTML = "<span><span style=\"color:red\"><input type=\"button\" style=\"white-space: normal; text-align: justify\" onclick=\"fnToggoleAnswerExplainDisplay();\" value=\"本题答案的解释 【注：建议基于本题答案解释的具体需求，超链接合适的字符文档（例如.docx文件、.wps文件）、图像PPT（例如.pptx文件）、视频（例如.mp4文件）、2D（例如.svg文件）、3D（例如.x3d文件）视媒作为主导】\" /></span>" + "<span style=\"display:none\" id=\"idExplain" + sTimeStamp + Math.floor(((i + 9) / 9)) + "\""+">" + cTr[i].getElementsByTagName("td").item(0).innerHTML + "</span></span>"; }
       if ((i + 9) % 9 === 7) { cTr[i].getElementsByTagName("td").item(0).innerHTML = "<span><span style=\"color:red\"><input type=\"button\" style=\"white-space: normal; text-align: justify\" onclick=\"fnToggoleFourLayerLanguageDisplay();\" value=\"本题选用的“四层平台”的思维语言的解释【注：同一目录条目的作业测验，各个题目选用的思维语言，一般差别不大】\" /></span>" + "<span style=\"display:none\" id=\"idToggoleFourLayerLanguageDisplay" + sTimeStamp + Math.floor(((i + 9) / 9)) + "\""+">" + cTr[i].getElementsByTagName("td").item(0).innerHTML + "</span></span>"; }
        if ((i + 9) % 9 === 8) { cTr[i].getElementsByTagName("td").item(0).innerHTML = "<span><span style=\"color:red\"><input type=\"button\" style=\"white-space: normal; text-align: justify\" onclick=\"fnToggoleFiveLayerMVCExplainDisplay();\" value=\"本题选用的“四层平台”的思维语言生成的“五层MVC”面向的主要层次【注：A、实践-数据读写封装（例如：人物对象的内容方法←映射→数据库数据仓库数据集成）(课程思政：学思结合、知行统一、实事求是，等等)；B、技术-信息提取运用（例如：数据确定性描述统计。典型案例：数据总计平均等等多维分析）（课程思政：大国工匠、精益求精、工程伦理、科技报国，等等）；C、科学-规律预测探究（例如：数据概率性推断统计。典型案例：数据挖掘）（课程思政：正确认识问题、分析问题、解决问题、科学思维、科学伦理，等等）；D、人文-情感交流共鸣（例如：数据概率性人文推断统计。典型案例：数据人文挖掘）（课程思政：学习兴趣、情感态度、人本主义、国家情怀、文化自信，等等）；E、哲学-智能建构生成（例如：数据概率性AIGC推断统计。典型案例：神经元多层网络的已经训练学习的概率性推断统计）（课程思政：辩证思辨、自主思辨、自主学习优化，等等）】\" /></span>" + "<span style=\"display:none\" id=\"idFiveLayerMVCExplain" + sTimeStamp + Math.floor(((i + 9) / 9)) + "\""+">" + cTr[i].getElementsByTagName("td").item(0).innerHTML + "</span></span>"; }
       // }
    }
...
var sUrl=window.location.href.substring(0,window.location.href.indexOf("/webCourse/"));
//open(sHref,"_blank");  
    window.open(sUrl+"/webCourse/options/HomeworkAndTestFiveLayerMVC.htm","winHomeworkAndTestFiveLayerMVC", "scrollbars=,width=800,height=600,top=" + (screen.height - 600) / 2 + ",left=" + (screen.width - 800) / 2);
}
