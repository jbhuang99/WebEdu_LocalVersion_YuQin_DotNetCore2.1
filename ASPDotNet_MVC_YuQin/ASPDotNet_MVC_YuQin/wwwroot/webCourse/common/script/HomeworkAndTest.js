  ...
  var oTable = document.getElementsByTagName("table").item(0);var cTr = Array.from(oTable.rows); // 只包含主表的直接<tr>。解决上述包含嵌套表格的行。
   ...
   var iProblemNum = iTrLenth / 8;
    ...
    for (i = 0; i < iTrLenth; i++) {
        ...
        if ((i + 8) % 8 === 5) {cTr[i].getElementsByTagName("td").item(0).innerHTML = "<span><span style=\"color:red\"><input type=\"button\" title='（本机端作业与测试不支持成绩统计分析，服务端作业与测验可支持成绩统计分析！）（本机端作业与测试←可以登录服务端同步→服务端作业与测验！）' onclick=\"fnAnswerAndToggoleAnswerDisplay();\" value=\"查看本题答案 【注：（本机端作业与测试不支持成绩统计分析，服务端作业与测验可支持成绩统计分析！）（本机端作业与测试←可以登录服务端同步→服务端作业与测验！）】\" /></span>" + "<span style=\"display:none\" id=\"idAnswer" + sTimeStamp + Math.floor(((i + 8) / 8))  +"\""+">" + cTr[i].getElementsByTagName("td").item(0).innerHTML + "</span></span>";}
       if ((i + 8) % 8 === 6) { cTr[i].getElementsByTagName("td").item(0).innerHTML = "<span><span style=\"color:red\"><input type=\"button\" onclick=\"fnToggoleAnswerExplainDisplay();\" value=\"查看本题答案的解释 【注：建议基于本题答案解释的具体需求，超链接合适的字符（例如.docx文件、.pptx文件）-图像视频（例如.mp4文件）-2D（例如.svg文件）-3D（例如.x3d文件）】\" /></span>" + "<span style=\"display:none\" id=\"idExplain" + sTimeStamp + Math.floor(((i + 8) / 8)) + "\""+">" + cTr[i].getElementsByTagName("td").item(0).innerHTML + "</span></span>"; }
        if ((i + 8) % 8 === 7) { cTr[i].getElementsByTagName("td").item(0).innerHTML = "<span><span style=\"color:red\"><input type=\"button\" onclick=\"fnToggoleFiveLayerMVCExplainDisplay();\" value=\"查看本题所属的五层MVC类别 【注：A、实践-数据读写封装；B、技术-信息提取运用；C、科学-规律预测探究；D、人文-情感交流共鸣；E、哲学-智能建构生成】\" /></span>" + "<span style=\"display:none\" id=\"idFiveLayerMVCExplain" + sTimeStamp + Math.floor(((i + 8) / 8)) + "\""+">" + cTr[i].getElementsByTagName("td").item(0).innerHTML + "</span></span>"; }
       // }
    }
