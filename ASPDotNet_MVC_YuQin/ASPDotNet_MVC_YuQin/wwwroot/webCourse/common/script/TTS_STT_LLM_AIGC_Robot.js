...
//window.localLLMID=""; // 本机LLM的名称//已经保存在后端
//window.localLLMURL=""; // 本机LLM的URL。暂时无需使用。
            ...
            xmlHttpRequest.onreadystatechange = function () {  //如果readyState发生变化的时候执行的函数
                ...
                if (xmlHttpRequest.readyState == 4) {  //ajax为4说明执行完了
                    ...
                    if (xmlHttpRequest.status == 200) { //如果是200说明成功
                        //如果函数存在的话执行 
                        window.localLLMURL=xmlHttpRequest.responseText.split("|||")[1];
                        oTemp.textContent="当前本机LLM是"+xmlHttpRequest.responseText.split("|||")[0]+"已运行，等候客户端的Prompt..."; 
                        window.localLLMID = xmlHttpRequest.responseText.split("|||")[0];                    
                    }
                    else {
                ...
                if (xmlHttpRequest.readyState == 4) {  //ajax为4说明执行完了
                    ...
                    if (xmlHttpRequest.status == 200) { //如果是200说明成功
                        ...
                        if (sTemp == "") {
                            ...
                            document.getElementById("idRunableLocalLLM").innerHTML ="<ol><li>"+sTemp.slice(0, -"|||".length).replace(/(\|\|\|)+/g, '<input type="radio" name="raio_RunableLocalLLM" onclick="fnRadioRunLocalLLM()" onblur="fnBlurRadioRunLocalLLM()"/><span>请单选运行LLM...</span></li><li>')+'<input type="radio" name="raio_RunableLocalLLM" onclick="fnRadioRunLocalLLM()" onblur="fnBlurRadioRunLocalLLM()"/><span>请单选运行LLM...</span></li></ol>';//将约定的"|||"替换为HTML的换行标签
                        }
