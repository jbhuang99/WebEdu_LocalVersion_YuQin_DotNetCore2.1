...
opener.parent.document.getElementById("sIframeContent").contentWindow.document.getElementById(elementId).remove();
    ...
    opener.parent.document.getElementById("sIframeContent").contentWindow.document.body.insertAdjacentHTML("afterbegin","<div id="+"\""+elementId+"\""+" style=\"color:green;text-align:justify\">LLM正在生成课文，请耐心等候...</div>");
    ...
    //opener.parent.document.getElementById("sIframeContent").contentWindow.document.getElementById(elementId).innerHTML="<div style=\"color:green\">LLM正在生成课文，请耐心等候...</div>";
...
function fnHomeworkAndTestInsertedElementForAIGC(elementId){
    ...
    //oCell.insertAdjacentHTML("beforeend","<div id="+"\""+elementId+"\""+" style=\"color:green;text-align:justify\">LLM正在生成作业测验，请耐心等候...</div>");
    oCell.getElementsByTagName("span").item(0).insertAdjacentHTML("afterend","<div id="+"\""+elementId+"\""+" style=\"color:green;text-align:justify\">LLM正在生成作业测验，请耐心等候...</div>");
    ...
    //opener.parent.document.getElementById("sIframeHomeworkAndTest").contentWindow.document.getElementById(elementId).innerHTML="<div style=\"color:green\">LLM正在生成作业测验，请耐心等候...</div>";
...
function fnAjaxServerSideCallCloudLLMExternal(isProxy){
    ...
    window.isRecognizingSystemExternal = false;
     ...
     let currentText = "视媒听媒机器人的回答Answer如下（请注意思辨字面准确性和语义准确性）（如果同时需要图像，建议千问免费生成图像后，在此切换成为编辑状态，然后可以复制粘贴在此）：";
     var endingAIGCedText = "（视媒听媒机器人的回答Answer已结束！）";
           ...
           xmlHttpRequest.send();////如果是post：xmlHttpRequest.setRequestHeader('content-type', 'application/x-www-form-urlencoded');  //设置请求头说明文档类型   xhr.send(data);  //send里传递数据
            xmlHttpRequest.onreadystatechange = function () {  //如果readyState发生变化的时候执行的函数
                ...
                if (xmlHttpRequest.readyState == 4) {  //ajax为4说明执行完了
                   ...
                   fnTTS_Play(0, "transcriptSystemExternal");
                     }
...
async function fnStreamChat(url,postData,targetIFrameId,targetElementId) {
  ...
  if (!res.ok) {
    ...
    target.innerHTML = '请求出错: ' + res.status + ' ' + err;
    return;
  }
      ...
      if (line.startsWith('data:')) {
        ...
        try {
          ...
          const token =
            ...
            parsed.choices?.[0]?.message?.content ||
            parsed;
          currentText += typeof token === 'string' ? token : JSON.stringify(token);
        } catch (e) {
          currentText += line + '\n';
        }
        target.innerHTML = '<div style="color:green">'+currentText+endingAIGCedText+'</div>';
...
function fnToggleDisplayOfAIGCAnswerHomeworkAndTest(targetElementId){
    if(document.getElementById("id_Answer" + targetElementId).style.display=="none"){
        document.getElementById("id_Answer" + targetElementId).style.display="block";
    }
    else{
    document.getElementById("id_Answer" + targetElementId).style.display="none";
    }
}

function fnToggleDisplayOfFiveLayMVCFromAIGCAnswerHomeworkAndTest(targetElementId){
    if(document.getElementById("id_FiveLayMVCFromAIGCAnswerHomeworkAndTest" + targetElementId).style.display=="none"){
        document.getElementById("id_FiveLayMVCFromAIGCAnswerHomeworkAndTest" + targetElementId).style.display="block";
    }
    else{
    document.getElementById("id_FiveLayMVCFromAIGCAnswerHomeworkAndTest" + targetElementId).style.display="none";
    }
}

function fnHTMLEditorForAIGCHomeworkAndTest(targetElementId){
    var sTemp="(HTML源码编辑)";
    if(document.getElementById("id_HTMLEditorForAIGCHomeworkAndTest"+targetElementId).textContent.indexOf(sTemp)>=0){
        document.getElementById("id_HTMLEditorForAIGCHomeworkAndTest"+targetElementId).textContent=document.getElementById("id_HTMLEditorForAIGCHomeworkAndTest"+targetElementId).textContent.replace(sTemp,"");
         document.getElementById("id_ForHTMLEditor"+targetElementId).innerHTML=document.getElementById("id_ForHTMLEditor"+targetElementId).textContent;
         document.getElementById("id_ForHTMLEditor"+targetElementId).contentEditable="false";
    }
    else{
    document.getElementById("id_HTMLEditorForAIGCHomeworkAndTest"+targetElementId).textContent=document.getElementById("id_HTMLEditorForAIGCHomeworkAndTest"+targetElementId). textContent+sTemp;
     document.getElementById("id_ForHTMLEditor"+targetElementId).textContent=document.getElementById("id_ForHTMLEditor"+targetElementId).innerHTML;
     document.getElementById("id_ForHTMLEditor"+targetElementId).contentEditable="true";
    }  
    }
        ...
        window.speechSynthesis.cancel();
                   ...
                   fnTTS_Play(0, "transcriptSystemInternal");
                        ...
                        alert(sTempErr);
                         window.speechSynthesis.cancel();
                     //TTS
                     const utteranceInternalAIGCAnswerOnError = new SpeechSynthesisUtterance("视媒听媒机器人的回答Answer如下（请注意思辨字面准确性和语义准确性）（如果同时需要图像，建议千问免费生成图像后，在此切换成为编辑状态，然后可以复制粘贴在此）"+sTempErr); 
                     window.speechSynthesis.speak(utteranceInternalAIGCAnswerOnError); 
                    }
                }
        }
...
function fnTTS_Play(intCharBeginningNumber,targetElementId) {
  ...
  //  if(document.getElementById("id_RadioSystemExternal").checked == true) {
   ...
   var sTemp="当前AIGC回答Answer朗读已结束，请继续Prompt提问AIGC！";
    if(!("speechSynthesis" in window)) {
       ...
       var sTextContent = document.getElementById(targetElementId).textContent;
       document.getElementById("id_CharNumber").textContent=sTextContent.length;
...
function fnTTS_GoTo() {
    window.speechSynthesis.cancel();
    fnTTS_Play(document.getElementById("id_TTS_GoToText").value, "transcriptSystemExternal");
}
  ...
  function fnToggleSystemInternalPPTImageSystemExternalPPTImage(){ 
      if(document.getElementById("iframeForPPTImageInternal").style.display=="none"){
            ...
            document.getElementById("iframeForPPTImageInternal").src = "../options/Accuracy-Similarity/Cone-of-Experience-FromKnowledgebase-willTransformToFiveLayeredMVC.jpg";
             document.getElementById("iframeForPPTImageExternal").src = "../options/Accuracy-Similarity/Cone-of-Experience-FromAIGC-willTransformToFiveLayeredMVC.jpg";
            }
      else{
 ...
 **/
  ...
  function fnPPTVideoInsertedElementForAIGC(elementId,videoId){
    var pptVideoIFrameSrc = document.getElementById(videoId).src;
    var oWindowContents=opener.parent.document.getElementById("sIframeContents").contentWindow;   
...
opener.parent.document.getElementById("sIframeContent").contentWindow.document.getElementById(elementId).remove();
    ...
    opener.parent.document.getElementById("sIframeHomeworkAndTest").contentWindow.document.getElementById(elementId).remove();
     ...
     opener.parent.document.getElementById("sIframeContent").contentWindow.document.body.insertAdjacentHTML("afterbegin","<div style=\"text-align:center\"><iframe src=\""+pptVideoIFrameSrc+"\" style=\"border:0;width: 100%; height: 718px;\" /></div>");
   // alert("已插入元素的innerHTML是："+ opener.parent.document.getElementById("sIframeContent").contentWindow.document.getElementById(elementId).outerHTML);      
    var oCell = opener.parent.document.getElementById("sIframeHomeworkAndTest").contentWindow.document.getElementsByTagName("table").item(0).rows[0].cells[0];
    oCell.getElementsByTagName("span").item(0).insertAdjacentHTML("afterend","<div style=\"text-align:center\"><iframe src=\""+pptVideoIFrameSrc+"\" style=\"border:0;width: 100%; height: 718px;\" /></div>");
   // alert("已插入元素的innerHTML是："+ opener.parent.document.getElementById("sIframeHomeworkAndTest").contentWindow.document.getElementById(elementId).outerHTML);
    }
  }
   function fnPPT2DInsertedElementForAIGC(elementId,twoDId){
    var ppt2DIFrameSrc = document.getElementById(twoDId).src;
    var oWindowContents=opener.parent.document.getElementById("sIframeContents").contentWindow;   
...
opener.parent.document.getElementById("sIframeContent").contentWindow.document.getElementById(elementId).remove();
   ...
   opener.parent.document.getElementById("sIframeContent").contentWindow.document.body.insertAdjacentHTML("afterbegin","<div style=\"text-align:center\"><iframe src=\""+ppt2DIFrameSrc+"\" style=\"border:0;width: 100%; height: 718px;\" /></div>");     
    var oCell = opener.parent.document.getElementById("sIframeHomeworkAndTest").contentWindow.document.getElementsByTagName("table").item(0).rows[0].cells[0];
    oCell.getElementsByTagName("span").item(0).insertAdjacentHTML("afterend","<div style=\"text-align:center\"><iframe src=\""+ppt2DIFrameSrc+"\" style=\"border:0;width: 100%; height: 718px;\" /></div>");
    }
  }

      function fnPPT3DInsertedElementForAIGC(elementId,ThreeDId){
    var ppt3DIFrameSrc = document.getElementById(ThreeDId).src;
    var oWindowContents=opener.parent.document.getElementById("sIframeContents").contentWindow;   
...
opener.parent.document.getElementById("sIframeContent").contentWindow.document.getElementById(elementId).remove();
   ...
   opener.parent.document.getElementById("sIframeContent").contentWindow.document.body.insertAdjacentHTML("afterbegin","<div style=\"text-align:center\"><iframe src=\""+ppt3DIFrameSrc+"\" style=\"border:0;width: 100%; height: 718px;\" /></div>");     
    var oCell = opener.parent.document.getElementById("sIframeHomeworkAndTest").contentWindow.document.getElementsByTagName("table").item(0).rows[0].cells[0];
    oCell.getElementsByTagName("span").item(0).insertAdjacentHTML("afterend","<div style=\"text-align:center\"><iframe src=\""+ppt3DIFrameSrc+"\" style=\"border:0;width: 100%; height: 718px;\" /></div>");
    }
  }
   function fnPPTAudioInsertedElementForAIGC(elementId,audioId){
    var pptAudioIFrameSrc = document.getElementById(audioId).src;
    var oWindowContents=opener.parent.document.getElementById("sIframeContents").contentWindow;   
...
opener.parent.document.getElementById("sIframeContent").contentWindow.document.getElementById(elementId).remove();
   ...
   opener.parent.document.getElementById("sIframeContent").contentWindow.document.body.insertAdjacentHTML("afterbegin","<div style=\"text-align:center\"><iframe src=\""+pptAudioIFrameSrc+"\" style=\"border:0;width: 100%; height: 50px;\" /></div>");     
    var oCell = opener.parent.document.getElementById("sIframeHomeworkAndTest").contentWindow.document.getElementsByTagName("table").item(0).rows[0].cells[0];
    oCell.getElementsByTagName("span").item(0).insertAdjacentHTML("afterend","<div style=\"text-align:center\"><iframe src=\""+pptAudioIFrameSrc+"\" style=\"border:0;width: 100%; height: 50px;\" /></div>");
    }
  }
