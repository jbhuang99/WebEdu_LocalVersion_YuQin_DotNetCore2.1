    ...
    public sealed partial class MainWindow : Window
    {
        ...
        private async void MainGrid_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Use the stable LanguageModel instance (experimental adapter support removed)
        ...
        private async void Generate_Click(object sender, RoutedEventArgs e)
        {
            ...
            // Experimental LoRA adapter loading removed.
            // Use stable options surface (if needed) or pass null for now.
            object? options = null;
            ...
            try
            {
                // Generate a response using the stable model/context (adapter-less)
                await GetResponse(_languageModel, LoraTxt, InitTxt, options);
            }
            catch (Exception ex)
            {
                InitTxt.Text = "Error: " + ex.Message;
            ...
            try
            {
                await GetResponse(_languageModel, NoLoraTxt, InitTxtNoLora, options);
            }
            catch (Exception ex)
            {
                InitTxtNoLora.Text = "Error: " + ex.Message;
                Console.WriteLine(ex);
            }
        }

        // Adapter/experimental-specific APIs removed. This implementation is a stable, minimal fallback
        // that echoes prompt/context to the UI. Replace with actual generation calls when stable APIs
        // are available and you want behavior similar to the original.
        async Task GetResponse(LanguageModel session, TextBlock textBox, TextBlock statusBlock, object? options)
        {
            try
            {
                statusBlock.Text = "Generating (adapter support removed)...";

                // Minimal fallback: show the prompt and system context in the output.
                // Replace this with actual generation calls to `context` or `session` when ready.
                var display = prompt;
                if (string.IsNullOrWhiteSpace(display) && context != null)
                {
                    display = "No prompt provided. Context is set.";
                }

                // Truncate to avoid extremely long UI content
                if (display.Length > 20000)
                {
                    display = display.Substring(0, 20000) + "...(truncated)";
                }

                // Update UI
                textBox.Text = display;
                statusBlock.Text = "Done (adapter not used).";
            }
            catch (Exception ex)
            {
                statusBlock.Text = "Error generating response: " + ex.Message;
                Debug.WriteLine(ex);
            }
